...
def run(imbalanceTechnique, featureTrans, featureSelection, clfName, data_all, label_all, x, z):
    ...
    if (clfName == "OneVsRestRF"):
        ...
        '''
        注意：这里设置bootstrap= True还是False有很大的不同。默认是True，我的Chapter7的实验结果都是基于bootstrap= True的。如果设置bootstrap= True，那么你在single decision tee会发现samples的数量并不等于values array的数字之和，这是因为此时设置了bootstrap= True，那么是有放回的采样。而如果设置bootstrap= False，则，会发现samples的数量等于values array的数字之和
        '''
        clf = OneVsRestClassifier(RandomForestClassifier(random_state=SEED, max_depth=3, bootstrap= True,n_jobs=-1))
    ...
    for each in range(NUM_RUNS):
        ...
        for trainindex, testindex in folds_list:
            ...
            if (clfName == "best"):
                ...
                decision_tree_estimator = clf_fit.steps[2][1].estimators_[0].estimators_[0]
                print("decision_tree_estimator:",decision_tree_estimator)

                print("path:", path)
                from sklearn.tree import export_graphviz
                # Export as dot file
                '''
                dot_data = export_graphviz(decision_tree_estimator, out_file=path+'/slaTree.dot',
                                feature_names=FEATURES_LIST,
                                class_names=TARGET_NAMES,
                                rounded=True, proportion=False,
                                precision=3, filled=True,max_depth=3, impurity=False,label="all")
                '''

                dot_data = export_graphviz(decision_tree_estimator,
                                           feature_names=FEATURES_LIST,
                                           class_names=TARGET_NAMES,
                                           rounded=True, proportion=False,
                                           precision=3, filled=True, max_depth=3, impurity=False, label="all")
                ...
                print("pydot_graph:",pydot_graph)
                pydot_graph.write_png(path + '/original_tree.png')
                pydot_graph.set_size('"100,100!"')
                pydot_graph.write_png(path + '/resized_tree.png')

                export_graphviz(decision_tree_estimator, out_file=path + '/slaTree.dot',
                                feature_names=FEATURES_LIST,
                                class_names=TARGET_NAMES,
                                rounded=True, proportion=False,
                                precision=3, filled=True, max_depth=3, impurity=False, label="all")
            ...
            if (featureSelection == "No_FeatureSelection"):
                randomforest_inside = clf_fit.steps[2][1].estimator
...
if __name__ == '__main__':
    ...
    for resampling in resampingMethod[1:2]:
