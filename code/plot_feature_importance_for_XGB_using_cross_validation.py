...
FEATURES_LIST = ['X3:num_tasks', 'X4:num_instances',
                 'X1:cpu_requested', 'X2:mem_requested',
                 'X5:real_cpu_max',
                 'X6:real_cpu_avg',
                 'X7:real_mem_max',
...
'''
FEATURES_LIST = ['X3_num_tasks', 'X4_num_instances',
                 'X1_cpu_requested', 'X2_mem_requested',
                 'X5_real_cpu_max',
                 'X6_real_cpu_avg',
                 'X7_real_mem_max',
                 'X8_real_mem_avg', 'X9_cpu_capacity',
                 'X10_mem_capacity']
'''
...
def plot_FeatureImportance(seqOfRun, imbalanceTechnique, featureTrans, featureSelection, clfName,
                           features_contributions,importance_type):
    plt.clf()
    ...
    myfig.savefig(
        path + "/figures/featureImportance_" +importance_type +"_" + str(
            seqOfRun) + "_" + imbalanceTechnique + "_" + featureTrans + "_" + featureSelection + "_" + clfName + ".jpg",
        dpi=DPI, format="jpg", bbox_inches='tight')
...
#this function is to change the output of get_booster().get_score(importance_type=importance_type)) into a list.
#input:{'T1_V8': 3021, 'T1_V11': 5060, 'T1_V16': 7310, 'T1_V2': 16499, 'T1_V5': 4860, 'T1_V1': 19240, 'T2_V4': 10578, 'T2_V14': 4649, 'T2_V2': 11513, 'T1_V17': 1065, 'T1_V12': 2450, 'T1_V10': 5044, 'T1_V4': 6462, 'T1_V15': 2264, 'T2_V15': 7107, 'T2_V10': 5674, 'T2_V6': 3150, 'T1_V3': 8557, 'T2_V1': 17076, 'T2_V9': 10191, 'T1_V13': 4002, 'T2_V3': 1493, 'T1_V14': 2869, 'T2_V5': 3041, 'T2_V13': 3556, 'T1_V7': 2153, 'T2_V7': 5242, 'T1_V6': 2056, 'T1_V9': 1658, 'T2_V8': 846, 'T2_V11': 1353, 'T2_V12': 530}
#output: 按照FEATURES_LIST的顺序[19240, 16499, 8557, 6462, 4860, 2056, 2153, 3021, 1658, 5044, 5060, 2450, 4002, 2869, 2264, 7310, 1065, 17076, 11513, 1493, 10578, 3041, 3150, 5242, 846, 10191, 5674, 1353, 530, 3556, 4649, 7107]
def changeDict2List(dict_importance):
    print("9999999999dict_importance:",dict_importance)
    temp = []
    for each in FEATURES_LIST:
        temp.append(dict_importance[each])
    return temp
...
def run(imbalanceTechnique, featureTrans, featureSelection, clfName, data_all, label_all, x, z):
    ...
    if (clfName == "OneVsRestRF"):
        ...
        clf = OneVsRestClassifier(xgb.XGBClassifier(random_state=SEED))
        ...
        baseclf = xgb.XGBClassifier(random_state=SEED)
    ...
    for each in range(NUM_RUNS):
        ...
        Y_test = cross_val_predict(pipeline, X_data, Y_data, cv=folds_for_evaluation,method="predict")
        ...
        for trainindex, testindex in folds_for_grid_search.split(data_all_np, label_all_np):
            ...
            if (featureSelection == "No_FeatureSelection"):
                importance_type = "cover"  #在XGB中衡量特征重要性的指标有3种（weight，gain，cover）。默认是weight
                xgb_inside = clf_fit.steps[2][1].estimator
                xgb_inside.fit(X_train, Y_train)
                features_contributions = changeDict2List(xgb_inside.get_booster().get_score(fmap='/media/sf_Downloads/codehome/mylabcode/sla/C10/xgb.fmap',importance_type=importance_type))
                ...
                features_list.append(features_contributions)
        ...
        features_df.to_excel(path + "/output/features_contributions_" + importance_type + ".xlsx")
...
clfMethod = ["OneVsRestRF","OneVsRestXGB","zeroRule","baggingXGB",'guess2', "SVC", "GradientBoostingClassifier", "best", "guess", "OneVsRestLR", "KerasNN", "PureRF", "OneVsRestRFClassWeight5", "OneVsRestRFClassWeight10",
