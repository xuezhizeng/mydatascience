...
def run(imbalanceTechnique, featureTrans, featureSelection, clfName, data_all, label_all,x,z):
    ...
    if (clfName == "guess"):
        ...
        if (imbalanceTechnique == 'No_Resampling'):
            # pipeline = Pipeline(steps = [('estimator', clf)])
            pipeline = imbPipeline([
                ('scaler', scaler),
                ('clf', clf)
            ])
            print("555555----pipeline-55555:", pipeline)
        else:
            print("6666---imbalanceHandler---66666:", imbalanceHandler)
            # pipeline = make_pipeline(imbalanceHandler,clf)
            # pipeline = Pipeline(steps=[('sampling', imbalanceHandler), ('estimator', clf)])
            # pipeline = make_pipeline(imbalanceHandler, clf)
            # pipeline = make_pipeline(SMOTE(random_state=SEED), clf)
            pipeline = imbPipeline([
                ('scaler', scaler),
                ('samplingMethod', imbalanceHandler),
                ('clf', clf)
            ])
            print("777777----pipeline-777777:", pipeline)
    ...
    folds_for_grid_search_new = StratifiedKFold(n_splits=SPLITS, shuffle=True, random_state=0)




    metrics_list = ["accuracy","balanced_accuracy","f1","f1_macro","roc_auc"]
    for m in metrics_list:
        cross_val_score(pipeline, data_all_np, label_all_np, cv=folds_for_grid_search_new, scoring=m)
    ...
    for each in range(NUM_RUNS):
        ...
        for trainindex, testindex in folds_for_grid_search.split(data_all_np, label_all_np):
            ...
            pr_area_Y_pred = plotPRcurve(i, imbalanceTechnique, featureTrans, featureSelection, clfName,Y_test, Y_test_pred_proba[:,1], 1)
            ...
            # auc_area_Y_pred = plot_ROC(i, imbalanceTechnique, featureTrans, featureSelection, clfName, 1,Y_test_Encoding, Y_pred_Encoding)

            # pr_area_Y_pred = plotPRcurve(i, imbalanceTechnique, featureTrans, featureSelection, clfName,Y_test_Encoding, Y_pred_Encoding, 1)

            '''
            if (featureSelection == "No_FeatureSelection"):
                randomforest_inside = tuned_model.best_estimator_.estimator
                randomforest_inside.fit(X_data, Y_data)
        ...
        '''
        temp2 = calAllMetrics2(imbalanceTechnique, featureTrans, featureSelection, clfName, accuracy_statics,
                       precision_macro_statics,precision_micro_statics, precision_weighted_statics, recall_macro_statics, recall_micro_statics,
                       recall_weighted_statics,f1_macro_statics,f1_micro_statics,f1_weighted_statics,f2_macro_statics,f2_micro_statics,f2_weighted_statics)
        '''
        print("===========hello===========")
...
'''
resampingMethod = ["No_Resampling","RandomUnderSampler","ClusterCentroids","NearMiss1","NearMiss2","NearMiss3","OnesidedSelcection","RandomOverSampler","SMOTE_regular","SMOTE_svm","SMOTE_borderline1","SMOTE_borderline2","SMOTE_ENN","SMOTE_Tomek"]
scalerMethod = ["No_FeatureTrans","StandardScaler","MinMaxScaler","Normalizer"]
featureSelMethod = ["No_FeatureSelection","RFSelect","SelectKBest","SVR"]
clfMethod = ["OneVsRestRF","PureRF"]
'''
