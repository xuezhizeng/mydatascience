...
def plot_xgb(clf, path):
    ...
    def _parse_node(graph, text):
        ...
        if match is not None:
            ...
            graph.node(node, label=match.group(2), shape='circle',color="white",style="filled")
    ...
    print("the number of trees:",len(booster.get_dump(fmap=path + '/xgb.fmap')))
    tree = booster.get_dump(fmap=path + '/xgb.fmap')[5]  # 这里 tree = booster.get_dump(fmap='xgb.fmap')[0], fmap就是前面生成的fmap文件, [0]表示第一棵树, 如果你想要其他树, 修改这个数字即可.
    ...
    graph.render(path + '/XGBoost_tree.png')
...
def run(imbalanceTechnique, featureTrans, featureSelection, clfName, data_all, label_all, x, z):
    ...
    if (clfName == "OneVsRestRF"):
        ...
        clf = OneVsRestClassifier(RandomForestClassifier(random_state=SEED, max_depth=3, n_jobs=-1))
        ...
        clf = OneVsRestClassifier(xgb.XGBClassifier(random_state=SEED,max_depth=3,n_estimators=4))
    ...
    for each in range(NUM_RUNS):
        ...
        for trainindex, testindex in folds_list:
            ...
            if (clfName == "best"):
                ...
                print("path:", path)
                print("type of clf_fit.steps[2][1].estimators_[0]:", type(clf_fit.steps[2][1].estimators_[0]))
                print("clf_fit.steps[2][1].estimators_[0]:", clf_fit.steps[2][1].estimators_[0])

                ceate_feature_map(path,FEATURES_LIST)
                plot_xgb(clf_fit.steps[2][1].estimators_[0],path)
...
if __name__ == '__main__':
    ...
    for clf in clfMethod[1:2]:  # to choose OneVsRestXGB
